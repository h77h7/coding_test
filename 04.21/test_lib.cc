...
uint64_t vta(
  ...
  void* vta_alu_handle = VTAMapRegister(VTA_ALU_ADDR);
  ...
  VTAWriteMappedReg(vta_alu_handle, 0x0, 0x81);
  ...
  VTAUnmapRegister(vta_alu_handle);
  ...
  for (int i = 0; i < num_insn; i++) {
    ...
    if (c.mem.opcode == VTA_OPCODE_LOAD || c.mem.opcode == VTA_OPCODE_STORE) {
      ...
      if (c.mem.opcode == VTA_OPCODE_STORE) {
        ...
        //if (c.mem.pop_next_dep) g2s_queue--;
      ...
      printf("DEBUG - g2a_queue = %d, a2g_queue = %d\n", g2a_queue, a2g_queue);
      printf("DEBUG - s2a_queue = %d, a2s_queue = %d\n", s2a_queue, a2s_queue);
      ...
      if (c.alu.pop_prev_dep) g2a_queue--;
      if (c.alu.push_prev_dep) a2g_queue++;
      if (c.alu.pop_next_dep) s2a_queue--;
      if (c.alu.push_next_dep) a2s_queue++;
      printf("DEBUG - l2g_queue = %d, g2l_queue = %d\n", l2g_queue, g2l_queue);
      printf("DEBUG - g2a_queue = %d, a2g_queue = %d\n", g2a_queue, a2g_queue);
      printf("DEBUG - s2a_queue = %d, a2s_queue = %d\n", s2a_queue, a2s_queue);
    }
...
int alu_test(int opcode, bool use_imm, int batch, int vector_size, bool uop_compression) {
  ...
  for (int b = 0; b < batch; b += VTA_BATCH) {
    ...
    insn_buf[insn_idx++] = get2DLoadStoreInsn(
        ...
        1,                                 // pop prev dep
        0,                                 // pop next dep
        b < batch - VTA_BATCH,             // push prev dep
        0);                                // push next dep
  }
  // Finish
