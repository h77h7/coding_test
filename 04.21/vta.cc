...
#pragma HLS INTERFACE axis port = alu_queue
...
void gemm(
  ...
  /*uop_T uop_mem_c[VTA_UOP_BUFF_DEPTH]*/) {
#pragma HLS INLINE

  static bus_T out_mem[VTA_ACC_BUFF_DEPTH][OUT_MAT_AXI_RATIO];
...
#pragma HLS PIPELINE II = 2
        ...
        for (int i = 0; i < VTA_BATCH; i++) {
          for (int b = 0; b < VTA_BLOCK_OUT; b++) {
            ...
            } else if (insn.alu_opcode == VTA_ALU_OPCODE_NOOP) {
	      o_tensor[i][b] = (out_T) src_0.range(VTA_OUT_WIDTH - 1, 0);
	    }
          }
        }
...
void compute(
  ...
  hls::stream<bool> &a2g_dep_queue,
  hls::stream<bool> &g2l_dep_queue,
  hls::stream<bool> &g2a_dep_queue,
  bus_T inp_mem[VTA_INP_BUFF_DEPTH][INP_MAT_AXI_RATIO],
  bus_T wgt_mem[VTA_WGT_BUFF_DEPTH][WGT_MAT_AXI_RATIO],
  //bus_T out_mem[VTA_ACC_BUFF_DEPTH][OUT_MAT_AXI_RATIO],
  uop_T uop_mem[VTA_UOP_BUFF_DEPTH],
...
#pragma HLS INTERFACE axis port = g2l_dep_queue
#pragma HLS INTERFACE axis port = g2a_dep_queue
#pragma HLS INTERFACE axis port = a2g_dep_queue
#pragma HLS INTERFACE bram port = inp_mem
#pragma HLS INTERFACE bram port = wgt_mem
//#pragma HLS INTERFACE bram port = out_mem
#pragma HLS INTERFACE bram port = uop_mem
#pragma HLS INTERFACE bram port = acc_mem
#pragma HLS INTERFACE s_axilite port = return bundle = CONTROL_BUS
#pragma HLS RESOURCE variable = inp_mem core = RAM_1P
#pragma HLS RESOURCE variable = wgt_mem core = RAM_1P
//#pragma HLS RESOURCE variable = out_mem core = RAM_1P
#pragma HLS RESOURCE variable = uop_mem core = RAM_1P
#pragma HLS RESOURCE variable = acc_mem core = RAM_1P

  // Micro-op storage
  //static uop_T uop_mem_c[VTA_UOP_BUFF_DEPTH];

  // Accumulator storage
  //static bus_T acc_mem_c[VTA_ACC_BUFF_DEPTH][ACC_MAT_AXI_RATIO];
//#pragma HLS ARRAY_RESHAPE variable = acc_mem_c complete dim=2
// This is necessary to obtain II=1
//#pragma HLS DEPENDENCE variable = acc_mem_c inter false
  ...
  if (insn.generic.opcode == VTA_OPCODE_FINISH) {
    ...
    gemm(raw_copy, uop_mem, acc_mem, inp_mem, wgt_mem);
  } 
...
void alu(
	hls::stream<insn_T> &alu_queue,
	hls::stream<bool> &g2a_dep_queue,
  hls::stream<bool> &s2a_dep_queue,
  hls::stream<bool> &a2g_dep_queue,
	hls::stream<bool> &a2s_dep_queue,
	bus_T out_mem[VTA_ACC_BUFF_DEPTH][OUT_MAT_AXI_RATIO],
  uop_T uop_mem[VTA_UOP_BUFF_DEPTH],
  bus_T acc_mem[VTA_ACC_BUFF_DEPTH][ACC_MAT_AXI_RATIO]) {
#pragma HLS INTERFACE axis port = alu_queue
#pragma HLS INTERFACE axis port = g2a_dep_queue
#pragma HLS INTERFACE axis port = a2g_dep_queue
#pragma HLS INTERFACE axis port = a2s_dep_queue
#pragma HLS INTERFACE axis port = s2a_dep_queue
#pragma HLS INTERFACE bram port = out_mem
#pragma HLS INTERFACE bram port = uop_mem
#pragma HLS INTERFACE bram port = acc_mem
#pragma HLS INTERFACE s_axilite port = return bundle = CONTROL_BUS
...
//#pragma HLS DEPENDENCE variable = acc_mem inter false
  ...
  // Pop GEMM instruction
	insn_T raw_insn = alu_queue.read();
	// Cast to GenericInsn
	VTAInsn insn;
	insn_T raw_copy = raw_insn;
	insn.generic = *((VTAGenericInsn *)&raw_copy);
	...
	alu_(raw_copy, uop_mem, acc_mem, out_mem);
	

	// Push dependence token if instructed
	if (insn.generic.push_prev_dep) {
		a2g_dep_queue.write(1);
	}
	if (insn.generic.push_prev_dep) {
		a2s_dep_queue.write(1);
	}
}
void store(
	...
	hls::stream<bool> &s2a_dep_queue,
  bus_T out_mem[VTA_ACC_BUFF_DEPTH][OUT_MAT_AXI_RATIO]//,
	//bus_T out_mem_c[VTA_ACC_BUFF_DEPTH][OUT_MAT_AXI_RATIO],
	/*bus_T out_mem_a[VTA_ACC_BUFF_DEPTH][OUT_MAT_AXI_RATIO]*/) {
#pragma HLS INTERFACE m_axi port = outputs offset = slave bundle = data_port
#pragma HLS INTERFACE axis port = store_queue
#pragma HLS INTERFACE axis port = a2s_dep_queue
//#pragma HLS INTERFACE axis port = g2s_dep_queue
#pragma HLS INTERFACE axis port = s2a_dep_queue
//#pragma HLS INTERFACE bram port = out_mem_c
//#pragma HLS INTERFACE bram port = out_mem_a
  #pragma HLS INTERFACE bram port = out_mem
#pragma HLS INTERFACE s_axilite port = return bundle = CONTROL_BUS
/*#pragma HLS RESOURCE variable = out_mem_c core = RAM_1P
#pragma HLS RESOURCE variable = out_mem_a core = RAM_1P*/
  #pragma HLS RESOURCE variable = out_mem core = RAM_1P
...
#pragma HLS INTERFACE s_axilite port = return bundle = CONTROL_BUS
  ...
  hls::stream<insn_T> tmp_alu_queue;
  PRAGMA_HLS(HLS stream depth = STREAM_IN_DEPTH variable = tmp_alu_queue)
  ...
  hls::stream<insn_T> alu_queue;
  PRAGMA_HLS(HLS stream depth = STREAM_IN_DEPTH variable = alu_queue)
  hls::stream<insn_T> store_queue;
  ...
  hls::stream<bool> g2a_dep_queue;
  PRAGMA_HLS(HLS stream depth = STREAM_IN_DEPTH variable=g2a_dep_queue)
  //hls::stream<bool> g2s_dep_queue;
  //PRAGMA_HLS(HLS stream depth = STREAM_IN_DEPTH variable=g2s_dep_queue)
  hls::stream<bool> a2s_dep_queue;
  PRAGMA_HLS(HLS stream depth = STREAM_IN_DEPTH variable=a2s_dep_queue)
  hls::stream<bool> s2a_dep_queue;
  PRAGMA_HLS(HLS stream depth = STREAM_IN_DEPTH variable=s2a_dep_queue)
  //hls::stream<bool> s2g_dep_queue;
  //PRAGMA_HLS(HLS stream depth = STREAM_IN_DEPTH variable=s2g_dep_queue)
  hls::stream<bool> a2g_dep_queue;
  PRAGMA_HLS(HLS stream depth=STREAM_IN_DEPTH variable=a2g_dep_queue)
  hls::stream<bool> g2l_dep_queue;
...
#pragma HLS DEPENDENCE variable = acc_mem inter false
  ...
  fetch(insn_count, insns, tmp_load_queue, tmp_gemm_queue, tmp_alu_queue, tmp_store_queue);
  ...
  while (true) {
    ...
    while (!tmp_gemm_queue.empty() || tmp_gemm_popped == true) {
      ...
      if (
        (insn.pop_prev_dep && !l2g_dep_queue.empty() &&
         insn.pop_next_dep && !a2g_dep_queue.empty()) ||
        ...
        compute(done, uops, biases, gemm_queue, l2g_dep_queue, a2g_dep_queue,
                g2l_dep_queue, g2a_dep_queue, inp_mem, wgt_mem, uop_mem, acc_mem);
      } else {
    ...
    // Next execute as many gemm instructions as possible
    while (!tmp_alu_queue.empty() || tmp_alu_popped == true) {
      // Pop the gemm instruction
      if (!tmp_alu_popped) {
        tmp_alu_queue.read(tmp_alu);
        tmp_alu_popped = true;
      }
      // Check dependences and invoke the load stage
      VTAGenericInsn insn = *((VTAGenericInsn *) &tmp_alu);
      if (
        (insn.pop_prev_dep && !g2a_dep_queue.empty() &&
         insn.pop_next_dep && !s2a_dep_queue.empty()) ||
        (!insn.pop_prev_dep && insn.pop_next_dep &&
         !s2a_dep_queue.empty()) ||
        (insn.pop_prev_dep && !g2a_dep_queue.empty() &&
        !insn.pop_next_dep) ||
        ...
        // Push the instruction in the load queue
        alu_queue.write(tmp_alu);
        tmp_alu_popped = false;
        alu(alu_queue, g2a_dep_queue, s2a_dep_queue,
                a2g_dep_queue, a2s_dep_queue, out_mem, uop_mem, acc_mem);
      } else {
        // Execution of load stage pending on completion of other stages,
        // so break here...
        break;
    ...
    while (!tmp_store_queue.empty() || tmp_store_popped == true) {
      ...
      if ((insn.pop_prev_dep && !a2s_dep_queue.empty()) ||
          !insn.pop_prev_dep) {
        // Push the instruction in the load queue
        store_queue.write(tmp_store);
        tmp_store_popped = false;
        store(outputs, store_queue, a2s_dep_queue, s2a_dep_queue, out_mem);
      } else {
        // Execution of load stage pending on completion of other stages, so break here...
        break;
      }
    ...
    if (exit_counter > 1000) {
      ...
      if (tmp_gemm_popped) {
        ...
        if (a2g_dep_queue.empty() && insn.pop_next_dep) {
          printf("waiting on a2g\n");
        }
      }
      if (tmp_alu_popped) {
          VTAGenericInsn insn = *((VTAGenericInsn *) &tmp_alu);
          if (g2a_dep_queue.empty() && insn.pop_prev_dep) {
              printf("waiting on g2a\n");
          }
          if (s2a_dep_queue.empty() && insn.pop_next_dep) {
              printf("waiting on s2a\n");
          }
      }
      if (tmp_store_popped) {
        if (a2s_dep_queue.empty()) {
          printf("waiting on a2s\n");
        }
      }
      break;
    }
  ...
  while (g2a_dep_queue.read_nb(tmp_tok)) {
    g2a_count++;
  }
  while (a2s_dep_queue.read_nb(tmp_tok)) {
    a2s_count++;
  }
  while (s2a_dep_queue.read_nb(tmp_tok)) {
    s2a_count++;
  }
  while (a2g_dep_queue.read_nb(tmp_tok)) {
    a2g_count++;
  }
  ...
  printf("l2g: %d, g2a: %d, a2s: %d, s2a: %d, a2g: %d, g2l: %d\n", l2g_count, g2a_count, a2s_count, s2a_count, a2g_count, g2l_count);
  assert(l2g_count == 0 && g2a_count == 0 && a2s_count == 0 && s2a_count == 0 && a2g_count == 0 && g2l_count == 0);
}
