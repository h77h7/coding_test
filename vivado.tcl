...
set acc_part	       $ACC_MEM_BANKS
set acc_mem_width	$ACC_MEM_WIDTH
set acc_mem_depth	$ACC_MEM_DEPTH
set uop_part	$UOP_MEM_BANKS
set uop_mem_width	$UOP_MEM_WIDTH
set uop_mem_depth	$UOP_MEM_DEPTH
...
set alu_base_addr $ALU_BASE_ADDR
...
set alu_ip "${ip_path}/vta_alu/soln/impl/ip/xilinx_com_hls_alu_1_0.zip"
...
update_ip_catalog -add_ip $alu_ip -repo_path $ip_lib
...
set alu_0 [ create_bd_cell -type ip -vlnv xilinx.com:hls:alu:1.0 alu_0 ]
...
set cmd_queue_list {load_queue gemm_queue store_queue alu_queue}
...
set dep_queue_list {l2g_queue g2l_queue g2s_queue s2g_queue g2a_queue a2s_queue}
...
# Create and connect out_mem_c partitions
for {set i 0} {$i < $out_part} {incr i} {
  # Create instance: out_mem, and set properties
  set out_mem_c [ create_bd_cell -type ip -vlnv xilinx.com:ip:blk_mem_gen:8.4 out_mem_c_${i} ]
  [ init_bram_property $out_mem_c $out_mem_width $out_mem_depth ]
  # If module has more than 1 mem port, the naming convention changes
  if {$out_part > 1} {
    set porta [get_bd_intf_pins compute_0/out_mem_${i}_V_PORTA]
    set portb [get_bd_intf_pins store_0/out_mem_c_${i}_V_PORTA]
  } else {
    set porta [get_bd_intf_pins compute_0/out_mem_V_PORTA]
    set portb [get_bd_intf_pins store_0/out_mem_c_V_PORTA]
  }
  # Create interface connections
  connect_bd_intf_net -intf_net compute_0_out_mem_${i}_V_PORTA \
    [get_bd_intf_pins $out_mem_c/BRAM_PORTA] \
    $porta
  connect_bd_intf_net -intf_net store_0_out_mem_${i}_V_PORTA \
    [get_bd_intf_pins $out_mem_c/BRAM_PORTB] \
    $portb
}

# Create and connect out_mem_a partitions
for {set i 0} {$i < $out_part} {incr i} {
  # Create instance: out_mem_a, and set properties
  set out_mem_a [ create_bd_cell -type ip -vlnv xilinx.com:ip:blk_mem_gen:8.4 out_mem_a_${i} ]
  [ init_bram_property $out_mem_a $out_mem_width $out_mem_depth ]
  # If module has more than 1 mem port, the naming convention changes
  if {$out_part > 1} {
    set porta [get_bd_intf_pins alu_0/out_mem_${i}_V_PORTA]
    set portb [get_bd_intf_pins store_0/out_mem_a_${i}_V_PORTA]
  } else {
    set porta [get_bd_intf_pins alu_0/out_mem_V_PORTA]
    set portb [get_bd_intf_pins store_0/out_mem_a_V_PORTA]
  }
  # Create interface connections
  connect_bd_intf_net -intf_net alu_0_out_mem_${i}_V_PORTA \
    [get_bd_intf_pins $out_mem_a/BRAM_PORTA] \
    $porta
  connect_bd_intf_net -intf_net store_0_out_mem_a_${i}_V_PORTA \
    [get_bd_intf_pins $out_mem_a/BRAM_PORTB] \
    $portb
}

# Create and connect acc_mem partitions
for {set i 0} {$i < $acc_part} {incr i} {
  # Create instance: acc_mem, and set properties
  set acc_mem [ create_bd_cell -type ip -vlnv xilinx.com:ip:blk_mem_gen:8.4 acc_mem_${i} ]
  [ init_bram_property $acc_mem $acc_mem_width $acc_mem_depth ]
  # If module has more than 1 mem port, the naming convention changes
  if {$acc_part > 1} {
    set porta [get_bd_intf_pins compute_0/acc_mem_${i}_V_PORTA]
    set portb [get_bd_intf_pins alu_0/acc_mem_${i}_V_PORTA]
  } else {
    set porta [get_bd_intf_pins compute_0/acc_mem_V_PORTA]
    set portb [get_bd_intf_pins alu_0/acc_mem_V_PORTA]
  }
  # Create interface connections
  connect_bd_intf_net -intf_net compute_0_acc_mem_${i}_V_PORTA \
    [get_bd_intf_pins $acc_mem/BRAM_PORTA] \
    $porta
  connect_bd_intf_net -intf_net alu_0_acc_mem_${i}_V_PORTA \
    [get_bd_intf_pins $acc_mem/BRAM_PORTB] \
    $portb
}

# Create and connect uop_mem partitions
for {set i 0} {$i < $uop_part} {incr i} {
  # Create instance: uop_mem, and set properties
  set uop_mem [ create_bd_cell -type ip -vlnv xilinx.com:ip:blk_mem_gen:8.4 uop_mem_${i} ]
  [ init_bram_property $uop_mem $uop_mem_width $uop_mem_depth ]
  # If module has more than 1 mem port, the naming convention changes
  if {$uop_part > 1} {
    set porta [get_bd_intf_pins compute_0/uop_mem_${i}_V_PORTA]
    set portb [get_bd_intf_pins alu_0/uop_mem_${i}_V_PORTA]
  } else {
    set porta [get_bd_intf_pins compute_0/uop_mem_V_PORTA]
    set portb [get_bd_intf_pins alu_0/uop_mem_V_PORTA]
  }
  # Create interface connections
  connect_bd_intf_net -intf_net compute_0_uop_mem_${i}_V_PORTA \
    [get_bd_intf_pins $uop_mem/BRAM_PORTA] \
    $porta
  connect_bd_intf_net -intf_net alu_0_uop_mem_${i}_V_PORTA \
    [get_bd_intf_pins $uop_mem/BRAM_PORTB] \
    $portb
}
...
connect_bd_intf_net -intf_net axi_xbar_M03_AXI [get_bd_intf_pins axi_xbar/M03_AXI] [get_bd_intf_pins alu_0/s_axi_CONTROL_BUS]
connect_bd_intf_net -intf_net axi_xbar_M04_AXI [get_bd_intf_pins axi_xbar/M04_AXI] [get_bd_intf_pins store_0/s_axi_CONTROL_BUS]
...
connect_bd_intf_net -intf_net fetch_0_alu_queue_V_V [get_bd_intf_pins fetch_0/alu_queue_V_V] [get_bd_intf_pins alu_queue/S_AXIS]
...
connect_bd_intf_net -intf_net compute_0_g2a_dep_queue_V [get_bd_intf_pins compute_0/g2a_dep_queue_V] [get_bd_intf_pins g2a_queue/S_AXIS]
...
connect_bd_intf_net -intf_net alu_0_a2s_dep_queue_V [get_bd_intf_pins alu_0/a2s_dep_queue_V] [get_bd_intf_pins a2s_queue/S_AXIS]
...
connect_bd_intf_net -intf_net alu_queue_M_AXIS [get_bd_intf_pins alu_0/alu_queue_V_V] [get_bd_intf_pins alu_queue/M_AXIS]
...
connect_bd_intf_net -intf_net g2a_queue_M_AXIS [get_bd_intf_pins alu_0/g2a_dep_queue_V] [get_bd_intf_pins g2a_queue/M_AXIS]
connect_bd_intf_net -intf_net a2s_queue_M_AXIS [get_bd_intf_pins store_0/a2s_dep_queue_V] [get_bd_intf_pins a2s_queue/M_AXIS]
...
connect_bd_net -net proc_sys_reset_peripheral_aresetn \
  ...
  [get_bd_pins axi_xbar/M04_ARESETN] \
  ...
  [get_bd_pins g2a_queue/s_aresetn] \
  [get_bd_pins a2s_queue/s_aresetn] \
...
connect_bd_net -net processing_system_clk \
  ...
  [get_bd_pins g2a_queue/s_aclk] \
  [get_bd_pins a2s_queue/s_aclk] \
  $maxi_clk \
  $saxi_clk
...
create_bd_addr_seg -range $ip_reg_map_range -offset $alu_base_addr [get_bd_addr_spaces processing_system/Data] [get_bd_addr_segs alu_0/s_axi_CONTROL_BUS/Reg] SEG_alu_0_Reg
