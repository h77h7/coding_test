...
enum PipelineStage : int { kNoneStage = 0, kLoadStage = 1, kComputeStage = 2, kStoreStage = 3, kAluStage = 4 };
    ...
    for (int i = 0; i < insn_count; ++i) {
      ...
      if (c.mem.opcode == VTA_OPCODE_LOAD || c.mem.opcode == VTA_OPCODE_STORE) {
        if (c.mem.x_size == 0) {
          ...
          if (c.mem.opcode == VTA_OPCODE_STORE) {
            ...
            if (c.mem.pop_next_dep) a2s_queue--;
          ...
          printf("\ta2s_queue = %d, g2a_queue = %d\n", a2s_queue, g2a_queue);
          continue;
        }
      ...
      if (c.mem.opcode == VTA_OPCODE_LOAD || c.mem.opcode == VTA_OPCODE_STORE) {
        if (c.mem.opcode == VTA_OPCODE_STORE) {
          ...
          CHECK(c.gemm.pop_next_dep == false);
          CHECK(c.gemm.push_next_dep == false);
          if (c.gemm.pop_prev_dep) g2s_queue--;
          if (c.gemm.push_prev_dep) a2s_queue++;
      }
      printf("\tl2g_queue = %d, g2l_queue = %d\n", l2g_queue, g2l_queue);
      printf("\ts2g_queue = %d, g2s_queue = %d\n", s2g_queue, g2s_queue);
      printf("\tg2s_queue = %d, a2s_queue = %d\n", g2s_queue, a2s_queue);
    }
  }
